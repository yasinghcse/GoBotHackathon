...
public class CouncilPDFCrawler {
		...
		try {
			...
			if (classContents != null) {
				...
				while (itElements.hasNext()) {
					...
					// found the assumed PARAGRAPH, lets do a few testings to
					// make sure
					if (tagElement.tagName().equalsIgnoreCase("p")) {
						...
						if (meetingHeader.contains("city") && meetingHeader.contains("council")
							...
							// if contains all these words we will assume that
							// this paragraph will contain the link to the
							// Agenda
							...
							while (itElementsParagraph.hasNext()) {
								...
								// Found a link, lets do a few tests to make
								// sure
								if (tagElementParagraph.tagName().equalsIgnoreCase("a")) {
									...
									if (containsProperDate(meetingDate)) {
										councilAgenda = new CouncilAgenda(tagElementParagraph.text(),
												tagElementParagraph.absUrl("href"));
			...
			// A connection happened to the URL so it is considered to be a
			// BROKEN URL
			// In this case this NODE is marked as BAD it may not be considered
			// while building the anything on this node.
			ex.printStackTrace();
		}
	...
	private static boolean containsProperDate(String meetingDate) {
		return (meetingDate.contains("january") || meetingDate.contains("february") || meetingDate.contains("march")
				...
				|| meetingDate.contains("october") || meetingDate.contains("november")
				|| meetingDate.contains("december"))
				&& (meetingDate.contains("2013") || meetingDate.contains("2015") || meetingDate.contains("2016")
						|| meetingDate.contains("2017") || meetingDate.contains("2018") || meetingDate.contains("2019")
						|| meetingDate.contains("2020"))
				&& (meetingDate.contains("1") || meetingDate.contains("2") || meetingDate.contains("3")
						|| meetingDate.contains("4") || meetingDate.contains("5") || meetingDate.contains("6")
						|| meetingDate.contains("7") || meetingDate.contains("8") || meetingDate.contains("9")
						|| meetingDate.contains("10") || meetingDate.contains("11") || meetingDate.contains("12")
						|| meetingDate.contains("13") || meetingDate.contains("14") || meetingDate.contains("15")
						|| meetingDate.contains("16") || meetingDate.contains("17") || meetingDate.contains("18")
						|| meetingDate.contains("19") || meetingDate.contains("20") || meetingDate.contains("21")
						|| meetingDate.contains("22") || meetingDate.contains("23") || meetingDate.contains("24")
						|| meetingDate.contains("25") || meetingDate.contains("26") || meetingDate.contains("27")
						|| meetingDate.contains("28") || meetingDate.contains("29") || meetingDate.contains("30")
						|| meetingDate.contains("31"));
	}
	...
	public static void main(String[] args) {
		// CouncilAgenda councilAgenda =
		// CouncilPDFCrawler.getNextCouncilAgenda();
		// System.out.println(councilAgenda);
